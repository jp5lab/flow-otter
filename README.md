...
- **Author** — `plan_flow` (methodology spine), `add_node` (generic — handles core types AND node-red-contrib-\* long tail), `add_dashboard_widget` (24 Dashboard 2.0 widgets including ui-chart, ui-gauge, ui-table, ui-button, ui-button-group, ui-form, ui-template, ui-notification, ui-control, ui-text), `add_subflow_instance`, `add_group` (including position/size/parent/info geometry), `add_comment`, `wire_nodes`, `set_wires`, `set_links`, `remove_node`, `update_node`, `move_node`, `discard_staged_change` (clears a pending stage; single-slot staging refuses to overwrite), `create_subflow_definition`, `instantiate_template`.
