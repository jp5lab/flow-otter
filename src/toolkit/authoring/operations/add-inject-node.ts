...
export interface AddInjectNodeOpts {
  /** Visible label (≤ 24 chars). Defaults to 'Inject'. */
  label?: string;
  /** Custom key. Auto-generated as `inject` (with collision suffix) if omitted. */
  key?: string;
  /** Node-RED inject passthrough (payload, payloadType, topic, repeat, crontab, etc.). */
  passthrough?: Readonly<Record<string, unknown>>;
  /** Membership in an existing group. */
  groupKey?: string;
}

export interface AddInjectNodeResult {
  spec: AuthoringSpec;
  newNodeKey: string;
}
...
class AddInjectNodeError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'AddInjectNodeError';
  }
}
...
export function addInjectNode(
  spec: AuthoringSpec,
  tabId: string,
  opts: AddInjectNodeOpts = {},
): AddInjectNodeResult {
  const tabIndex = spec.tabs.findIndex((t) => t.id === tabId);
  if (tabIndex < 0) throw new AddInjectNodeError(`Tab '${tabId}' not found in spec.`);
