...
export interface AddStatusNodeOpts {
  /** Visible label (≤ 24 chars). Defaults to 'Status'. */
  label?: string;
  /** Custom key. Auto-generated as `status` (with collision suffix) if omitted. */
  key?: string;
  /** Node-RED status passthrough (scope, etc.). */
  passthrough?: Readonly<Record<string, unknown>>;
  /** Membership in an existing group. */
  groupKey?: string;
}

export interface AddStatusNodeResult {
  spec: AuthoringSpec;
  newNodeKey: string;
}
...
class AddStatusNodeError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'AddStatusNodeError';
  }
}
...
export function addStatusNode(
  spec: AuthoringSpec,
  tabId: string,
  opts: AddStatusNodeOpts = {},
): AddStatusNodeResult {
  const tabIndex = spec.tabs.findIndex((t) => t.id === tabId);
  if (tabIndex < 0) throw new AddStatusNodeError(`Tab '${tabId}' not found in spec.`);
