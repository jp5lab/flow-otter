...
export interface AddLinkOutNodeOpts {
  /** Visible label (≤ 24 chars). Defaults to 'Link Out'. */
  label?: string;
  /** Custom key. Auto-generated as `link-out` (with collision suffix) if omitted. */
  key?: string;
  /** Node-RED link-out passthrough (links, linkType: 'static' | 'dynamic', etc.). */
  passthrough?: Readonly<Record<string, unknown>>;
  /** Membership in an existing group. */
  groupKey?: string;
}

export interface AddLinkOutNodeResult {
  spec: AuthoringSpec;
  newNodeKey: string;
}
...
class AddLinkOutNodeError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'AddLinkOutNodeError';
  }
}
...
export function addLinkOutNode(
  spec: AuthoringSpec,
  tabId: string,
  opts: AddLinkOutNodeOpts = {},
): AddLinkOutNodeResult {
  const tabIndex = spec.tabs.findIndex((t) => t.id === tabId);
  if (tabIndex < 0) throw new AddLinkOutNodeError(`Tab '${tabId}' not found in spec.`);
