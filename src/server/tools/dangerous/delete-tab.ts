import { z } from 'zod';

import {
  ALL_DEPLOY_MODES,
  DEFAULT_DEPLOY_MODE,
  isDeployMode,
} from '../../../adapters/nodered/deploy.js';
import type { DeployMode } from '../../../shared/flow-source.js';
import type { FlowsJson } from '../../../shared/flows-json.js';
import { canonicalHash } from '../../../shared/hash.js';
import { type Tool, ValidationFailedError } from '../_tool.js';

import { assertDangerousToken, requireAllowedDeployMode } from './_confirmation.js';

const InputSchema = z
  .object({
    tab_id: z.string().min(1),
    confirmation_token: z.string().min(1),
    deploy_mode: z.enum(ALL_DEPLOY_MODES as unknown as [string, ...string[]]).optional(),
  })
  .strict();
type Input = z.infer<typeof InputSchema>;

const OutputSchema = z.object({
  ok: z.boolean(),
  deleted_tab_id: z.string(),
  removed_count: z.number(),
  new_hash: z.string(),
  previous_hash: z.string(),
  deployment_mode: z.enum(['full', 'nodes', 'flows', 'reload']),
  rev_before: z.string().nullable(),
  rev_after: z.string().nullable(),
  snapshot_before: z.string(),
});
type Output = z.infer<typeof OutputSchema>;

export const deleteTabTool: Tool<Input, Output> = {
  name: 'delete_tab',
  description:
    'Dangerous: deletes one tab and all nodes/groups/comments whose z field points at it. Requires ENABLE_DANGEROUS_TOOLS and a prepare_dangerous_operation token scoped to the tab id.',
  tier: 'dangerous',
  inputZod: InputSchema,
  inputJsonSchema: {
    type: 'object',
    properties: {
      tab_id: { type: 'string', minLength: 1 },
      confirmation_token: { type: 'string', minLength: 1 },
      deploy_mode: { type: 'string', enum: ALL_DEPLOY_MODES as unknown as string[] },
    },
    required: ['tab_id', 'confirmation_token'],
    additionalProperties: false,
  },
  outputZod: OutputSchema,
  handler: async (input, ctx) => {
    assertDangerousToken(input.confirmation_token, {
      operation: 'delete_tab',
      environment: ctx.config.ENVIRONMENT_NAME,
      actor: ctx.config.ACTOR_NAME,
      target: input.tab_id,
    });

    const deployMode: DeployMode = isDeployMode(input.deploy_mode)
      ? input.deploy_mode
      : DEFAULT_DEPLOY_MODE;
    requireAllowedDeployMode(deployMode, ctx.config.ALLOWED_DEPLOYMENT_MODES);

    const { flows: runtimeFlows, rev: runtimeRev } = await ctx.flowSource.load();
    const previousHash = canonicalHash(runtimeFlows);
    if (!runtimeFlows.some((node) => node.type === 'tab' && node.id === input.tab_id)) {
      throw new ValidationFailedError(`Tab '${input.tab_id}' not found.`, []);
    }

    const nextFlows: FlowsJson = runtimeFlows.filter((node) => {
      if (node.type === 'tab') return node.id !== input.tab_id;
      const z = (node as { z?: unknown }).z;
      return z !== input.tab_id;
    });
    const newHash = canonicalHash(nextFlows);

    const preSnap = await ctx.snapshots.save({
      flows: runtimeFlows,
      rev: runtimeRev,
      env: ctx.config.ENVIRONMENT_NAME,
      actor: ctx.config.ACTOR_NAME,
      reason: `pre-dangerous-delete-tab ${input.tab_id}`,
      takenAt: ctx.clock().toISOString(),
      tags: ['pre-dangerous', 'delete-tab'],
      serverVersion: ctx.serverVersion,
    });

    const saveOpts: { reason: string; deployMode: DeployMode; expectedRev?: string } = {
      reason: 'delete_tab',
      deployMode,
    };
    if (runtimeRev !== null) saveOpts.expectedRev = runtimeRev;
    const { rev: newRev } = await ctx.flowSource.save(nextFlows, saveOpts);

    ctx.enrichAudit({
      mode: 'dangerous',
      snapshot_before: preSnap.id,
      deployment_mode: deployMode,
    });

    return {
      ok: true,
      deleted_tab_id: input.tab_id,
      removed_count: runtimeFlows.length - nextFlows.length,
      new_hash: newHash,
      previous_hash: previousHash,
      deployment_mode: deployMode,
      rev_before: runtimeRev,
      rev_after: newRev || null,
      snapshot_before: preSnap.id,
    };
  },
};
