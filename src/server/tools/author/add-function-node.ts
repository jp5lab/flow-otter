...
import { addFunctionNode } from '../../../toolkit/authoring/operations/add-function-node.js';
...
export const addFunctionNodeTool: Tool<Input, Output> = {
  name: 'add_function_node',
  description:
    'Stages a new `function` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addFunctionNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addFunctionNode(priorSpec, tabId, opts);
