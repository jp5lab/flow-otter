...
import { addLinkCallNode } from '../../../toolkit/authoring/operations/add-link-call-node.js';
...
export const addLinkCallNodeTool: Tool<Input, Output> = {
  name: 'add_link_call_node',
  description: withStagedAuthorToolDescription(
    'Stages a new `link call` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  ),
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addLinkCallNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addLinkCallNode(priorSpec, tabId, opts);
