...
import { addCatchNode } from '../../../toolkit/authoring/operations/add-catch-node.js';
...
export const addCatchNodeTool: Tool<Input, Output> = {
  name: 'add_catch_node',
  description:
    'Stages a new `catch` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addCatchNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addCatchNode(priorSpec, tabId, opts);
