...
import { addLinkOutNode } from '../../../toolkit/authoring/operations/add-link-out-node.js';
...
export const addLinkOutNodeTool: Tool<Input, Output> = {
  name: 'add_link_out_node',
  description:
    'Stages a new `link out` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addLinkOutNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addLinkOutNode(priorSpec, tabId, opts);
