...
import { addCompleteNode } from '../../../toolkit/authoring/operations/add-complete-node.js';
...
export const addCompleteNodeTool: Tool<Input, Output> = {
  name: 'add_complete_node',
  description:
    'Stages a new `complete` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addCompleteNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addCompleteNode(priorSpec, tabId, opts);
