...
import { addInjectNode } from '../../../toolkit/authoring/operations/add-inject-node.js';
...
export const addInjectNodeTool: Tool<Input, Output> = {
  name: 'add_inject_node',
  description:
    'Stages a new `inject` node (an injection trigger node) on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec) => {
        ...
        const opts: Parameters<typeof addInjectNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addInjectNode(priorSpec, tabId, opts);
