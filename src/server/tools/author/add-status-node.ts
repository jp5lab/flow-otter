...
import { addStatusNode } from '../../../toolkit/authoring/operations/add-status-node.js';
...
export const addStatusNodeTool: Tool<Input, Output> = {
  name: 'add_status_node',
  description:
    'Stages a new `status` node on the given tab. Validates and lints the result; produces a semantic diff. Does NOT deploy — call `deploy_staged_change` to push to the runtime.',
  tier: 'author',
  ...
  handler: (input, ctx) =>
    runStagedAuthorOp<{ tabId: string; newNodeKey: string }, Output>(
      ...
      (priorSpec, priorFlows) => {
        ...
        const opts: Parameters<typeof addStatusNode>[2] = {};
        ...
        const { spec: nextSpec, newNodeKey } = addStatusNode(priorSpec, tabId, opts);
