...
FlowOtter v2.0.0 is the release cut for the 2026-06-10 layout-audit fix campaign. FULLY FIXED was declared on 2026-07-06 at commit `740dd5d`; the committed public audit record is [`docs/audits/2026-07-06-audit-rerun.md`](docs/audits/2026-07-06-audit-rerun.md).

- **Dependency security bumps (pre-publish, 2026-07-06):** `hono` 4.12.18 → 4.12.28 (transitive via the MCP SDK), `ws` 8.20.0 → 8.21.0 (uninitialized-memory disclosure + fragment-DoS advisories), and an `esbuild` override to ≥0.28.1 (dev-only dev-server advisory). `npm audit`: 0 vulnerabilities.
...
- **Contract parity for staged validation:** `validate_flow` and `validate_all_flows` now accept `against:'staged'|'runtime'` like the render tools. The default runtime path preserves the legacy response shape, while staged validation reads the pending staging slot, returns `staged_hash`/`based_on_snapshot_hash`, accepts staged tab `_authoringKey` or materialized id for `validate_flow`, and fails empty slots with `staging/no-staged-change`. This closes the live Phase-3 pre-deploy validation gap where agents staged a whole flow with `stage_spec` but final validation still checked only the deployed runtime.
- **EVAL-4 (scored)**: `npm run eval:s6 -- --scored` is live against the frozen thresholds — the runner verifies the thresholds.json/PROTOCOL.md sha256 freeze (refusing on any mismatch; `--superseded-thresholds <sha>` is the only escape and prints both SHAs plus the thresholds commit list in the gate verdict), cross-checks the frozen first-run lint baselines against the current linter (drift = hard fail), then scores every manifest×leg row through the new `layout-toolkit` engine adapter: leg A strips fixture placement and re-lays-out via `layoutFlowsJson`; leg B derives a zero-coordinate spec at runtime (`decompile` + `stripPositions`, recorded as `spec_source:"derived-zero-coordinate"`) through `layoutFlows`. Each row records raw `flowMetrics`, `layoutLint` scores, per-rule lint deltas, wiring-identity semantics, and crash status; the run record carries `{version, commit, engine, engine_version, thresholds_sha, seed}`. Blind judging packets gain real seeded-side SVG artifacts (baseline vs engine per packet); the answer key stays gitignored. Verdict PASS requires `not_worse_rate == 1`, zero crashes, and all-semantics-pass. Pure scoring math lives in `scripts/eval/benchmark/score.mjs` with boundary pins; the skeleton's not-yet-implemented `--scored` pin is replaced by scored-mode pins (hash-mismatch refusal, supersession, determinism, record shape).
- feat(author): add default-off `spec_authoring` toolset with `stage_spec` and `validate_spec`. `stage_spec` stages geometry-free whole-tab `AuthoringSpec` JSON through the shared compile/validate/lint/diff/stage choke point, preserves ids by `_authoringKey`, pins surviving prior geometry, computes new placement with the two-level engine, supports `dry_run` plus `amend_of`/`force_takeover`, and rejects raw `x`/`y`/`position`/`w`/`h` fields. `validate_spec` runs the same computed-placement diagnostics path as read-only output, works with pending stages and in `READ_ONLY_MODE`, and never writes the staging slot.
- feat(author): first-class `info` authoring — `NodeSpec.info` and `SubflowDefSpec.info` compile/decompile round-trip (property-covered), `add_node`/`update_node` gain `opts.info` (null clears), `plan_flow` recommends recording stage purpose as info, catalog notes Node-RED 4.1+ GitHub-style markdown alerts in info text (NR5-5)
- feat(author): `/settings` `nodeDefaults` merged into `add_node` materialization on runtimes with `nodeDefaultsOverride` (>=4.1.9) — merge order schema < settings < caller passthrough, re-validated; new `defaults_applied_from` output reports per-field provenance; file mode and older targets unchanged (NR5-6)
- feat(layout): add opt-in `layout_flow` author tool behind a default-off `layout` toolset. It stages one deterministic geometry-only tab layout through the standard staging pipeline, supports dry-run diff/report mode, validates all unknown tab/key references in one `ValidationFailedError`, applies non-persisted lane hints/section ordering for the two-level engine, preserves pinned geometry verbatim, and records the S6 gate as the condition for flipping the toolset default.
- feat(author): subflow per-instance config-node env authoring — `create_subflow_definition` declares typed env, `add_subflow_instance` gains `opts.env`; `conf-type` values resolve config-node authoring keys to compiled ids with a missing-ref diagnostic (values preserved verbatim, never dropped); gated on the `subflowPerInstanceConfig` capability when a runtime is connected (NR5-12)
- feat(render): `render_flow_svg` gains `highlight_unknown_types` — node types the runtime doesn't report render with the editor's `#fee` unknown-node fill (subflow instances whose definition exists in the flows are never flagged); `/nodes` is fetched only on request, degrading to normal rendering in file mode or on probe failure. Groups with non-empty `info` get a deterministic doc-badge glyph in the header area; with the option off and no group info, output is byte-identical to before (NR5-14)
- **tls-config authoring schema (NR5-9):** `add_node`/`add_config_node` now validate `tls-config` passthrough — `certType` `files|pfx|env` (5.0 pfx/env cert modes), `p12`/`p12name`, `certEnv`/`keyEnv`/`caEnv`, `servername` (SNI), `verifyservercert`, `alpnprotocol`; credential fields are never authored. http-request's `tls` field now documents that SNI comes from the referenced tls-config's `servername`.
- **MQTT v5 authoring (NR5-10):** `mqtt in` gains `nl`/`rap`/`rh`/`inputs` (dynamic-subscription mode), `mqtt out` gains `respTopic`/`contentType`/`correl`/`expiry`/`userProps`, and a full `mqtt-broker` config-node schema lands (`protocolVersion` 3|4|5, birth/close/will trios, `sessionExpiry`, `userProps`) — field names verified against the Node-RED 5.0.0 core node sources; broker credentials stay editor-only.
- **known-issues lint**: new `known-issues` validator warns when a function node calls `RED.util.getSetting(...)` and the target runtime is Node-RED 5.0.0-beta.6, 5.0.0, or 5.0.1 — the function-sandbox wrapper lost its return statement in 5.0.0-beta.6 and the call silently returns `undefined` (unfixed upstream as of 2026-07-06); the warning suggests `env.get(...)`. Backed by a KNOWN_ISSUES registry with exact affected-version lists and a 3-level member-path AST detector shared with the linkcall lint.
- **EVAL-4 (skeleton)**: S6 scoring-runner plumbing landed behind the LAYO-4 engine-adapter seam — `npm run eval:s6` dry-runs freeze + fixture hash verification, manifest×leg iteration (leg B skipped `pending-layo-2`), and deterministic seeded blind-packet generation with a gitignored answer key; `--scored` refuses on thresholds.json/PROTOCOL.md sha256 drift (hashes parsed from the docs, `--superseded-thresholds` prints both SHAs) and otherwise exits via a distinct not-yet-implemented path until the post-kill-switch engine API lands. Audit e4-probe `flowMetrics`/`stripPositions` promoted to pure modules under `scripts/eval/benchmark/`. Frozen thresholds untouched; all changes additive.
- **explain_flow link awareness (NR5-3 + NR5-4):** edges now carry `kind: 'wire' | 'link' | 'linkcall'`. Static `link out`/`link call` pairs (non-dynamic `links[]`) surface as `kind:'link'` virtual edges and function-node `node.linkcall(<literal>)` calls resolve to link-ins by id-then-name as `kind:'linkcall'` edges, with cross-tab hops called out in `notes[]`; virtual edges participate in entrypoint/sink/orphan classification. `analyze_flow`'s `linkSummary` gains an additive `functionLinkCalls` count, and the `link-resolution` validator now warns (severity `warning`) on literal `node.linkcall` targets that match no link-in; non-literal targets produce no edge and no warning.
- **New `add_config_node` tool (author toolset, NR5-8)** — stages global config nodes (`mqtt-broker`, `tls-config`, dashboard config) with `{key, type, label?, passthrough?}`; passthrough validates against NODE_SCHEMAS when the type is registered (`type_had_schema` surfaced); duplicate keys refused; compiled config entries carry no canvas fields (x/y/z/wires). Credentials are never authored: a `credentials` key in passthrough is rejected loudly here and on `add_node`'s legacy config-node path.
- **New `update_tab` tool (author toolset)** — stages partial updates to an existing tab's `label`, `info`, and typed `env` entries; `env` replaces wholesale (`[]` clears); requires at least one field; unknown tabs refused. Credential-typed env entries may be declared but non-empty values are rejected — FlowOtter does not author credential values.
...
- Header comments (associated to groups via sections.ts's `headerFor` reader seam) are placed above their group's box, grid-snapped, excluded from the group's geometry but included in stacking extents — closing the slice-A evidence's stranded-headers gap (multi-header fixture pinned: every header strictly above its group, x-overlapping it, none co-located, none at the origin).
- Group geometry write-back: groups receive `position` (top-left) + `size` from the ELK compound bounds, snapped outward to the 20px grid with a 40px minimum extent; a containment post-pass keeps members inside their boxes and parents around children (sibling groups pairwise disjoint, nested groups contained — both pinned).
- Lane partitioning is group-atomic: a group and all its descendants lay out in the group's majority lane (deterministic `LANE_ORDER` tie-break), so compounds are never split. Diagnostic rule names frozen at this merge: `layout/width-overflow` (warning), `layout/cross-lane-wire` (info, wire endpoints resolve to different lanes), `layout/group-spans-lanes` (warning, mixed-lane group). `layout/engine-error` behavior unchanged — ELK failure leaves the tab untouched; well-formed input never throws.
- Final post-pass grid-snaps node/junction/header centers and group corners, then translates the whole graph into bounds (no per-node clamping); relayout is a fixed point (pinned). Geometry still flows exclusively through the GeometryProvider helpers; the F10 grep-guard extends to the new files (no hardcoded width literals, `LANE_GAP` must be imported, never re-declared).
...
- Safety-spine behavior is shared, not forked: `based_on_snapshot_hash` is the batch-start runtime hash, deploy drift refusal and rollback remain unchanged, post-compile diagnostics abort the whole batch, and failed fold ops throw `BatchOpError` with WSB-1's reserved `failed_op_index` / `failed_op` payload fields while leaving the staging slot untouched. Same-batch remove-then-readd tombstones prevent compile's prior-id exact-match path from reusing removed ids, preserving batch ≡ sequential equivalence.
- The live-eval iterate loop is first-class without weakening single-op refusal: `stage_changes` accepts `amend_of` with the exact pending `staged_hash` to replace that stage; absent or stale hashes still hit the normal WSB-3 pending-stage guard. Registered in the default author toolset, documented in the tool reference, and re-added to `SERVER_INSTRUCTIONS` with the previously removed stage_changes sentence.
- Review finding closed: `stage_changes` amend now enforces the same cross-agent staging ownership guard as deploy/discard. A foreign-agent pending stage can be amended only with matching `amend_of` plus `force_takeover:true`; same-agent and pre-v0.6.0 no-`agent_id` stages keep the existing back-compat path, and non-amend calls cannot use `force_takeover` to bypass the normal pending-stage refusal.

### WSB-2 — Compile ID preservation

- **Cross-tab group/comment key ownership**: `compile()` now builds the complete ID-derivation entry list before resolving IDs, reserves every exact `container:kind:key` owner first, then runs the existing scoped fallback / legacy-id / fixed / fresh path in the prior deterministic order. This layers on top of 951994b's global/tab/subflow fallback scoping: same-scope cross-tab moves still preserve IDs, while a new same-key group/comment on an earlier tab can no longer steal the owner's ID or create duplicate IDs. Fresh hash IDs keep the legacy unsalted seed when available and deterministically salt (`~1`, `~2`, ...) when the base hash is already reserved or present in the prior flow. `add_group` and `add_comment` now expose optional `key` inputs through both Zod and JSON schemas and thread them to the existing op support, so agents can intentionally create matching authoring keys across tabs without tripping ID uniqueness.

### WSB-4 — Full object lifecycle for groups, comments, and junctions

- The authoring operation contract flip WSB-3 recorded as deferred is now complete: `moveNode`, `updateNode`, and `removeNode` are pure-and-throwing for missing targets, with the old silent `updated:false` / `removed:false` tests migrated. `move_node`, `update_node`, and `remove_node` now address regular nodes, junctions, and comments by authoring key or same-tab Node-RED id; cross-tab node/junction/comment moves preserve surviving ids, drop stale group membership, and scrub source-tab wires where applicable. Junction updates map native fields (`label`→`name`, `position`, `group_key`, `disabled`) and reject passthrough; comment updates through the legacy node updater are limited to text/position/group membership, with full comment editing on `update_comment`.
