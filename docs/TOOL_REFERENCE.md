...
Tool visibility depends on (a) tier flags and (b) **toolsets** (v1.3.0+). The registered surface is 75 tools. The default visible surface is 21 tools: 5 core plumbing tools plus 16 intent-shaped read, authoring, layout, and deploy tools. The remaining toolsets are either demoted-but-callable during the v2 deprecation window or explicitly opt-in.

Each tool surfaces MCP-spec annotation hints (`readOnlyHint` / `destructiveHint` / `idempotentHint` / `openWorldHint`) on `tools/list` for client UIs to communicate intent. Every tool also advertises an `outputSchema` and returns matching `structuredContent` on successful calls; the JSON text content remains for compatibility.

## Toolsets (progressive disclosure)

FlowOtter groups its tools into 12 named toolsets. Default-on: `core`, `essentials`, `spec_authoring`, `layout`, `deploy`. Default-off: `discovery`, `analyze`, `snapshots`, `audit`, `author`, `author_specialists`, `dangerous`. Call `list_available_toolsets` to inspect current visibility, hidden-callability, and supersession records; call `enable_toolset(name)` to make an off-list set visible in `tools/list`.

Demoted toolsets are hidden from `tools/list` by default since 2.0.0 but remain callable during the deprecation window. `discovery`, `analyze`, `snapshots`, and `audit` are superseded by the `essentials` default surface plus `enable_toolset` on demand. The per-op `author` toolset is superseded by `stage_spec` / `stage_changes` (`enable_toolset("author")` re-lists per-op editing). Removal is no earlier than 2.2.0.

`author_specialists` remains opt-in and is not callable while hidden. `dangerous` remains env-gated by the dangerous tier and is not callable while hidden; when the env gate enables it, destructive confirmation still applies.
...
- `analyze_flow`
- `analyze_all_flows`
- `validate_flow` — runs validators and layout scoring for one tab. `against: 'staged' | 'runtime'` (default `'runtime'`) selects the source: `'staged'` validates the pending staged change (fails with a `staging/no-staged-change` diagnostic when the slot is empty); `'runtime'` validates the deployed flows, which never include a pending stage. On the staged path, `tab_id` accepts either the tab's `_authoringKey` or its materialized Node-RED id. Omitted `against` preserves the legacy response shape; calls that supply `against` also return `against`, `staged_hash`, and `based_on_snapshot_hash` (both hashes null for runtime), with staged `rev` set to the staged change's `based_on_rev`.
- `validate_all_flows` — same validator/layout suite over the whole document. Supports the same `against` selector and empty-slot diagnostic as `validate_flow`; omitted `against` preserves the legacy response shape, while source-selected responses carry the same provenance fields.
- `render_flow_svg` — deterministic SVG of one tab. `against: 'staged' | 'runtime'` (default `'runtime'`) selects the source: `'staged'` renders the pending staged change (fails with a `staging/no-staged-change` diagnostic when the slot is empty); `'runtime'` renders the deployed flows, which never include a pending stage. Optional `highlight_unknown_types:true` fetches installed node types from the runtime and fills missing types with the editor unknown-node color; file mode or probe failure degrades to normal rendering. Output carries `against`, `staged_hash`, and `based_on_snapshot_hash` (both null for runtime renders); for staged renders `rev` is the runtime rev the stage was computed against (= `get_staged_change`'s `based_on_rev`).
...
The default authoring path is `stage_spec` or `stage_changes`, with `layout_flow` available by default for deterministic geometry. Per-op author tools remain callable during the v2 deprecation window but are hidden from `tools/list` until `enable_toolset("author")` is called.
...
- `add_node` — **generic node-add**. Takes `{tab_id, type, opts:{info?, passthrough?, source_node_id?, ...}}`. Validates passthrough against per-type Zod when registered (26 core types incl. inject/debug/function/mqtt/link/catch/status/complete); when `passthrough` is omitted and the schema's defaults satisfy it, runtime-required defaults (e.g. inject `repeat`, complete `scope`) are materialized automatically. On Node-RED 4.1.9+ targets, settings.js `nodeDefaults` are merged before caller passthrough (`schema < settings < passthrough`) and then re-validated. Output includes `defaults_applied_from`, a per-field map of defaults sourced from `schema` or `settings`. Accepts arbitrary passthrough for unknown types with `type_had_schema:false` hint. **Preferred default for canvas nodes** — handles every Node-RED core type AND the long tail of `node-red-contrib-*` packages (Modbus, InfluxDB, OPC UA, BACnet, S7, etc.) first-class. Use `add_config_node` for global config nodes and specialist tools from `author_specialists` only when type-specific schema validation matters.
- `add_config_node` — stages a global config node (`{key, type, label?, passthrough?}`), such as `mqtt-broker`, `tls-config`, or dashboard config nodes. Validates passthrough against NODE_SCHEMAS when a schema is registered and returns `type_had_schema`. Config nodes compile without canvas fields (`x`, `y`, `z`, `wires`). FlowOtter never authors credentials: a `credentials` key in passthrough is rejected.
...
- `add_subflow_instance` — stages a subflow instance on a tab. Input: `{tab_id, defId, opts:{label?, key?, passthrough?, env?, group_key?}}`. `opts.env` supplies per-instance env overrides using `{name,type,value?,ui?}` entries; for `type:"conf-type"`, `value` is a config-node authoring key and compiles to the config node's Node-RED id. On connected runtimes this is gated by `subflowPerInstanceConfig` (Node-RED 4.0+); file mode allows authoring without probing.
...
- `stage_changes` — atomic ordered batch staging. Input: `{ops:[...], reason?, dry_run?, amend_of?, force_takeover?}` with 1-200 ops (`add_node`, `add_junction`, `add_group`, `add_comment`, `wire_nodes`, `set_wires`, `set_links`, `remove_node`, `remove_comment`, `update_node`, `move_node`, `update_group`, `remove_group`, `update_comment`). The batch loads/decompiles once, folds ops left-to-right, then compiles/validates/lints/diffs/stages once; `dry_run:true` returns the same validation/diff result without writing the staging slot. References such as `source_node_id`, `node_id`, `from_key`/`to_key`, `node_keys`, `group_key`, and scoped/pinned keys resolve first against keys already present in the evolving batch spec, then against Node-RED ids from the runtime baseline. `move_node` uses `tab_id` plus optional `dest_tab_id` only. If a pending stage exists, pass `amend_of` with the exact current `staged_hash` to replace it during an iterate loop; if that stage was authored by a different agent process, add `force_takeover:true` intentionally. `force_takeover` applies only to matching `amend_of`; absent or wrong hashes use the normal stage-overwrite refusal.
- `layout_flow` — default-on layout tool that stages deterministic geometry for one tab. Input: `{tab_id, scope?, lane_hints?, section_order?, pinned?, engine?, dry_run?}`. `scope` is currently tab-level only; `lane_hints` (`main|indicate|error`) and `section_order` are per-run hints and are not persisted. `pinned` keys keep their geometry verbatim and translate laid content below the pinned bounding box. `engine` defaults to `two_level`; `legacy_dagre` is available but refuses `lane_hints`/`section_order` so inputs are never silently dropped. Unknown tab/keys are reported together in one `ValidationFailedError`. `dry_run:true` validates/diffs/reports without writing the staging slot.
- `stage_spec` — default-on declarative authoring tool that stages geometry-free `AuthoringSpec` JSON as ONE staged change. Input: `{spec, layout_hints?, reason?, dry_run?, amend_of?, force_takeover?}`. Scope semantics: each tab declared in `spec.tabs` is authoritative and replaced wholesale, so omitted nodes/groups/comments/junctions in that tab are removed; tabs omitted from `spec.tabs` are preserved. `configNodes` and `subflowDefs` are preserved when omitted and replaced wholesale when supplied. Node, junction, comment, and group specs reject raw `x`, `y`, `position`, `w`, and `h` fields, including geometry inside canvas-object `passthrough`; FlowOtter computes placement with the same two-level engine as `layout_flow`. Comment specs support optional `headerFor` with the target group authoring key, placing the comment above that group when resolved; unresolved values fall back silently to the normal header heuristic. Existing objects whose authoring keys match the runtime keep their Node-RED ids and pinned geometry; new spec-authored objects get computed grid placement. `layout_hints` supports `lane_hints` and `section_order`; for multi-tab specs put them under `layout_hints.per_tab.<tab_key>`. Pending-stage, `amend_of`, and `force_takeover` semantics match `stage_changes`.
- `validate_spec` — default-on read-only diagnostics for the same declarative spec path, including comment `headerFor` support. Input: `{spec, layout_hints?}`. It runs the same compile, validation, lint, diff, and computed-placement path as `stage_spec`, returns `would_stage_hash`, `diff_summary`, `diagnostics`, `has_errors`/`errors`/`warnings`, and `layout_report`, but never writes the staging slot and does not refuse because a staged change is already pending. It is read-tier, so it remains available in `READ_ONLY_MODE`.
- `remove_node` — removes a node, junction, or comment by key on a tab. Junction removal scrubs all connections touching it; all removals scrub visual group membership. Missing targets throw.
- `update_node` — updates a regular node, junction, or comment by key. Regular nodes support `info` (pass `null` to clear), full-property `passthrough` merge, and **line-based `patches[]`** for token-efficient edits to string properties (function-node `func`, ui-template `format`, template `template`). Patches are `{property, op:'replace'|'insert'|'delete', start, end?, content?}` with 1-indexed line numbers on the ORIGINAL content; non-overlapping. Junctions map `label`→`name`, `position`, `group_key`, and `disabled`; passthrough/info is rejected for junctions/comments.
- `update_tab` — updates an existing tab's `label`, `info`, and/or `env`. Input: `{tab_id, label?, info?, env?}`. Passing `env` replaces the tab env array wholesale; use `env:[]` to clear existing entries. Env entries are `{name,type,value?,ui?}` with `type` in `str|num|bool|json|env|cred|jsonata|conf-type`. Credential-typed env entries may be declared, but non-empty `value` is rejected because FlowOtter does not author credential values.
- `move_node` — repositions or cross-tab moves a node, junction, or comment. Takes `tab_id` (the tab currently holding the object — the same parameter vocabulary as every other author tool) plus optional `dest_tab_id` and `position`. `source_tab_id` is a DEPRECATED alias of `tab_id` (kept for back-compat, removal slated for v2.0.0); using it triggers the `param-vocabulary` soft nudge. Cross-tab moves drop group membership and scrub source-tab connections for nodes/junctions.
- `update_group` — updates an existing visual group. Supports `name`, `node_keys` (nodes/junctions/comments), `position`, `size`, `parent_key`, `info`, `style`, `passthrough`, and `refit:true`; refit strips explicit geometry so compile recomputes a deterministic auto-fit from current member positions.
- `remove_group` — removes a visual group with Node-RED ungroup semantics: members are never deleted; direct members and child groups reparent to the removed group's parent or become top-level.
- `update_comment` — updates an existing comment's `text`, `position`, `size`, `info`, and `group_key`; pass null for `size`, `info`, or `group_key` to clear.
- `create_subflow_definition` — stages a subflow definition. Input includes `{name, id?, nodes?, connections?, env?, passthrough?}`. `env` declares subflow-level typed env variables (`str|num|bool|json|env|cred|jsonata|conf-type`) and round-trips through the subflow definition's `env` array.
...
**Single-slot staging contract:** every author tool stages exactly one change computed against the live runtime. If a stage is already pending, author tools refuse with a pointer to `deploy_staged_change` / `discard_staged_change` — nothing is ever silently overwritten. `stage_changes` alone can explicitly amend a pending stage when `amend_of` byte-equals that stage's `staged_hash`, and the same cross-agent ownership guard applies unless `force_takeover:true` is supplied.
...
`validate_flow` / `validate_all_flows` run the full validator suite. Both accept `against: 'staged' | 'runtime'`; use `against:'staged'` for final pre-deploy checks because the default runtime path intentionally excludes pending staged work. The `get_authoring_guide(['validators'])` catalog enumerates each rule with severity and category. Notable v1.3.0 additions for operator-screen design:
