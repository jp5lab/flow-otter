...
Tool visibility depends on (a) tier flags and (b) **toolsets** (v1.3.0+). The registered surface is 73 tools. The default visible surface is ~54 tools; calling `enable_toolset('author_specialists')` reveals the 11 type-specific `add_*_node` conveniences; calling `enable_toolset('layout')` reveals the opt-in `layout_flow` tool. Dangerous tools require both `ENABLE_DANGEROUS_TOOLS=true` AND the `dangerous` toolset (auto-enabled when the env flag is set, +7 tools).
...
FlowOtter groups its tools into 10 named toolsets. Default-on: `core`, `discovery`, `analyze`, `snapshots`, `audit`, `author`, `deploy`. Default-off: `author_specialists`, `layout`, `dangerous`. The agent can introspect via `list_available_toolsets` and load additional sets via `enable_toolset(name)`.
