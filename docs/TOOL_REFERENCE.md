...
Tool visibility depends on (a) tier flags and (b) **toolsets** (v1.3.0+). The registered surface is 71 tools. The default visible surface is ~53 tools; calling `enable_toolset('author_specialists')` reveals the 11 type-specific `add_*_node` conveniences. Dangerous tools require both `ENABLE_DANGEROUS_TOOLS=true` AND the `dangerous` toolset (auto-enabled when the env flag is set, +7 tools).
